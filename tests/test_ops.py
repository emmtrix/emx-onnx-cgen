...
from emx_onnx_cgen.lowering.sequence_empty import lower_sequence_empty
...
def _make_random_uniform_like_model(
    *,
    input_shape: list[int],
    ...
    attrs: dict[str, object] = {"low": low, "high": high}
    ...
    graph = helper.make_graph(
        [node], "random_uniform_like_graph", [input_info], [output]
    )
    model = helper.make_model(
        graph,
        producer_name="onnx2c",
...
def _make_sequence_empty_model() -> onnx.ModelProto:
    output_sequence = helper.make_tensor_sequence_value_info(
        "output_sequence", TensorProto.FLOAT, shape=[3]
    )
    node = helper.make_node(
...
def _make_sequence_at_model() -> onnx.ModelProto:
    sequence = helper.make_tensor_sequence_value_info(
        "sequence", TensorProto.FLOAT, shape=[3]
    )
...
def _make_split_to_sequence_model(
    *,
    ...
    if with_split:
        ...
        if split_as_input:
            inputs.append(
                helper.make_tensor_value_info("split", TensorProto.INT64, [3])
            )
        else:
            split = helper.make_tensor(
                "split", TensorProto.INT64, dims=[3], vals=[2, 2, 2]
            )
...
def _make_group_query_attention_model(
    *,
    ...
    q_seq: int = 1,
    kv_seq: int = 1,
    seqlens_value: np.ndarray | None = None,
) -> onnx.ModelProto:
    q_hidden = num_heads * qk_head_size
    ...
    if seqlens_value is None:
        seqlens_value = np.zeros(seqlens_shape, dtype=np.int32)
    else:
        seqlens_value = seqlens_value.astype(np.int32)
    ...
    output = helper.make_tensor_value_info(
        "output", TensorProto.FLOAT, [batch, q_seq, q_hidden]
    )
    present_key = helper.make_tensor_value_info(
        "present_key",
        TensorProto.FLOAT,
...
def _run_gqa_testbench_compare(model: onnx.ModelProto) -> None:
    """Compare the compiled C testbench against onnxruntime for a GQA model.

    Unlike ``_run_testbench_compare`` this preserves static zero-sized input
    dimensions (the empty key/value tensors of the shared-KV case) instead of
    substituting 1, so onnxruntime receives the exact declared shapes.
    """
    initializer_names = {init.name for init in model.graph.initializer}
    rng = np.random.default_rng(0)
    inputs: dict[str, np.ndarray] = {}
    for value_info in model.graph.input:
        if value_info.name in initializer_names:
            continue
        dtype = _tensorproto_to_dtype(value_info.type.tensor_type.elem_type)
    ...
    for output_info, ort_output in zip(model.graph.output, ort_outputs):
        ...
        np.testing.assert_allclose(output_data, ort_output, rtol=1e-4, atol=1e-5)


def test_group_query_attention_empty_kv_shared_past_testbench_compare() -> None:
    model = _make_group_query_attention_model(
        kv_seq=0,
        with_past=True,
        max_seq_len=4,
        seqlens_value=np.array([2], dtype=np.int32),
    )
    _run_gqa_testbench_compare(model)


def test_group_query_attention_empty_kv_batched_testbench_compare() -> None:
    model = _make_group_query_attention_model(
        batch=2,
        kv_seq=0,
        with_past=True,
        max_seq_len=4,
        seqlens_shape=(2,),
        seqlens_value=np.array([1, 3], dtype=np.int32),
    )
    _run_gqa_testbench_compare(model)


def test_group_query_attention_empty_kv_gqa_ratio_testbench_compare() -> None:
    model = _make_group_query_attention_model(
        num_heads=8,
        kv_num_heads=1,
        kv_seq=0,
...
def test_group_query_attention_empty_kv_large_head_testbench_compare() -> None:
    model = _make_group_query_attention_model(
        qk_head_size=64,
        v_head_size=64,
        kv_seq=0,
...
def test_group_query_attention_empty_kv_prompt_causal_testbench_compare() -> None:
    # q_seq > 1 attending only into the past cache exercises the causal mask.
    model = _make_group_query_attention_model(
        q_seq=8,
        kv_seq=0,
        with_past=True,
        max_seq_len=8,
        seqlens_value=np.array([7], dtype=np.int32),
    )
    _run_gqa_testbench_compare(model)
...
def test_lower_sequence_empty() -> None:
    graph = import_onnx(_make_sequence_empty_model())
    op = lower_sequence_empty(graph, graph.nodes[0])
...
def test_lower_sequence_insert_rejects_shape_mismatch() -> None:
    sequence = helper.make_tensor_sequence_value_info(
        "sequence", TensorProto.FLOAT, shape=[2]
    )
    tensor = helper.make_tensor_value_info("tensor", TensorProto.FLOAT, [3])
    output_sequence = helper.make_tensor_sequence_value_info(
        "output_sequence", TensorProto.FLOAT, shape=[2]
    )
    node = helper.make_node(
        "SequenceInsert",
        inputs=["sequence", "tensor"],
    ...
    model = helper.make_model(
        helper.make_graph(
            [node],
            "sequence_insert_shape_mismatch_graph",
            [sequence, tensor],
            [output_sequence],
        ),
        producer_name="emx-onnx-cgen",
    ...
    onnx.checker.check_model(model)
    graph = import_onnx(model)
    with pytest.raises(ShapeInferenceError, match="tensor shape"):
        lower_sequence_insert(graph, graph.nodes[0])


def test_lower_sequence_at_rejects_shape_mismatch() -> None:
    sequence = helper.make_tensor_sequence_value_info(
        "sequence", TensorProto.FLOAT, shape=[2]
    )
    position = helper.make_tensor("position", TensorProto.INT64, dims=[1], vals=[0])
    output = helper.make_tensor_value_info("output", TensorProto.FLOAT, [3])
    node = helper.make_node(
        "SequenceAt", inputs=["sequence", "position"], outputs=["output"]
    )
    model = helper.make_model(
        helper.make_graph(
            [node],
            "sequence_at_shape_mismatch_graph",
            [sequence],
            [output],
            initializer=[position],
        ),
    ...
    onnx.checker.check_model(model)
    graph = import_onnx(model)
    with pytest.raises(ShapeInferenceError, match="output shape"):
        lower_sequence_at(graph, graph.nodes[0])
...
def test_nonzero_inference_stores_values_in_context() -> None:
    ...
    graph = import_onnx(model)
    load_lowering_registry()
    op = get_lowering("NonZero")(graph, graph.nodes[0])
...
def test_non_max_suppression_inference_uses_context_only() -> None:
    ...
    graph = import_onnx(model)
    load_lowering_registry()
    op = get_lowering("NonMaxSuppression")(graph, graph.nodes[0])
...
def test_nonzero_codegen_output_is_stable() -> None:
    ...
    assert compiler.compile(model) == compiler.compile(model)
...
def test_random_uniform_like_run_within_bounds() -> None:
    model = _make_random_uniform_like_model(
        input_shape=[2, 3],
        input_dtype=TensorProto.INT32,
        output_dtype=TensorProto.FLOAT,
        low=-1.0,
        high=0.5,
        seed=3.0,
    )
    input_data = np.arange(6, dtype=np.int32).reshape(2, 3)
    outputs = _run_reference(model, {"input": input_data})
    out = outputs["output"]
    assert out.shape == (2, 3)
    assert out.dtype == np.float32
...
def test_random_uniform_like_op_compiles() -> None:
    model = _make_random_uniform_like_model(
        input_shape=[1, 4],
        input_dtype=TensorProto.FLOAT,
        low=1.0,
    ...
    generated = Compiler(CompilerOptions()).compile(model)
    assert "RandomUniformLike" in generated
    assert "_rng_next_u64" in generated
