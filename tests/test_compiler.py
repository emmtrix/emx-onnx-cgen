...
def _make_split_dynamic_dim_model() -> onnx.ModelProto:
    return helper.make_model(
        helper.make_graph(
            ...
            inputs=[helper.make_tensor_value_info("x", TensorProto.FLOAT, ["N", 2, 3])],
            outputs=[
                helper.make_tensor_value_info("y0", TensorProto.FLOAT, ["N", 1, 3]),
                helper.make_tensor_value_info("y1", TensorProto.FLOAT, ["N", 1, 3]),
            ],
        ),
...
def _make_constant_of_shape_via_neg_model() -> onnx.ModelProto:
    ...
    return helper.make_model(
        helper.make_graph(
            ...
            inputs=[],
            outputs=[helper.make_tensor_value_info("out", TensorProto.FLOAT, ["N"])],
...
def _make_range_from_shape_gather_model() -> onnx.ModelProto:
    ...
    return helper.make_model(
        helper.make_graph(
            ...
            outputs=[helper.make_tensor_value_info("out", TensorProto.INT64, ["N"])],
        ...
        helper.make_graph(
            ...
            inputs=[
                helper.make_tensor_value_info(
                    "shape_src", TensorProto.FLOAT, [2, 3, 4]
                ),
...
def test_compile_debug_lowering_failure_context_disabled_by_default() -> None:
    ...
    with pytest.raises(
        ShapeInferenceError, match="Split does not support dynamic dims"
    ) as exc_info:
...
def test_compile_debug_lowering_failure_context_enabled() -> None:
    ...
    assert (
        "x: tensor[dtype=float, shape=(-1, 2, 3), dim_params=('N', None, None)]"
        in message
    )
    assert (
...
def test_compile_resolves_constant_of_shape_from_negated_int_list() -> None:
    generated = Compiler(CompilerOptions()).compile(
        _make_constant_of_shape_via_neg_model()
    )
...
def test_compile_resolves_range_length_from_shape_gather_chain() -> None:
    generated = Compiler(CompilerOptions()).compile(
        _make_range_from_shape_gather_model()
    )
...
def test_compile_allows_range_shape_inference_with_float_constants() -> None:
    generated = Compiler(CompilerOptions()).compile(
        _make_range_with_float_constants_model()
    )
...
def test_compile_resolves_reshape_target_from_rank_size_chain() -> None:
    generated = Compiler(CompilerOptions()).compile(
        _make_reshape_target_from_rank_size_model()
    )
