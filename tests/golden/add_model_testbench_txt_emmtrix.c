 ...
 * Attrs: n/a
 */
EMX_NODE_FN void node0_add(const float input0[2][3][4], const float input1[2][3][4], float output[2][3][4]) {
...
static void testbench_print_txt_emmtrix(
float out[2][3][4]) {
    for (idx_t i0 = 0; i0 < 2; ++i0) {
        for (idx_t i1 = 0; i1 < 3; ++i1) {
            for (idx_t i2 = 0; i2 < 4; ++i2) {
                printf("#ULP1000 %s[%zu] = %a\n", "out", (size_t)((i0 * 3 + i1) * 4 + i2), (double)out[i0][i1][i2]);
            }
        }
    }
}
...
static int testbench_run(const char *input_path) {
    ...
    testbench_print_txt_emmtrix(out);
    return 0;
}
