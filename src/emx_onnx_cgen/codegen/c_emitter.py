...
class CEmitter:
    ...
    def _collect_includes(
        ...
        *,
        ...
        if any(
            ...
            includes.add("#include <stdbool.h>")
        if any(
            isinstance(op, SoftmaxCrossEntropyLossOp) and op.ignore_index is not None
            for op in resolved_ops
        ):
            includes.add("#include <stdbool.h>")
        if any(
            isinstance(op, UnaryOp)
            and unary_op_symbol(op.function, dtype=model.op_context.dtype(op.output))
            in {"llabs", "abs"}
            for op in resolved_ops
        ):
        ...
        if any(
            isinstance(
                op, (ConcatOp, ReshapeOp, SplitOp, SplitToSequenceOp, IdentityOp)
            )
            for op in resolved_ops
        ):
    ...
    def _needs_math(
        ...
        if any(
            isinstance(op, UnaryOp)
            and unary_op_symbol(op.function, dtype=op_context.dtype(op.output))
            in math_ops
            for op in resolved_ops
        ):
            return True
        ...
        if any(
            ...
            return True
        if any(
            isinstance(op, BinaryOp) and is_binary_math_op(op) for op in resolved_ops
        ):
            return True
        if any(
            isinstance(
        ...
        if any(
            ...
            return True
        if any(isinstance(op, MaxPoolOp) and op.dtype.is_float for op in resolved_ops):
            return True
        if any(
            isinstance(
                op,
    ...
    def _render_reduce_like_op(
        ...
        *,
        ...
        if isinstance(op, ArgReduceOp):
            ...
            return templates["arg_reduce"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                output=params["output"],
    ...
    def _render_op(
        ...
        if isinstance(op, BinaryOp):
            ...
            input_dtype = self._ctx_dtype(op.input0)
            input1_dtype = (
                self._ctx_dtype(op.input1) if isinstance(op, PowOp) else input_dtype
            )
            output_dtype = self._ctx_dtype(op.output)
            params = self._shared_param_map(
                [
            ...
            rendered = templates["binary"].render(
                **common,
                input0=params["input0"],
        ...
        if isinstance(op, MultiInputBinaryOp):
            ...
            input_c_type = input_dtype.c_type
            output_c_type = output_dtype.c_type
            input_names = [params[f"input{idx}"] for idx in range(len(op.inputs))]
            ...
            input_exprs = [
                CEmitter._broadcast_index_expr(name, shape, output_shape_raw, loop_vars)
            ...
            rendered = templates["multi_input"].render(
                **common,
                inputs=input_names,
        ...
        if isinstance(op, WhereOp):
            ...
            rendered = templates["where"].render(
                model_name=model.name,
                op_name=op_name,
                output_shape=output_shape,
                loop_vars=loop_vars,
        ...
        if isinstance(op, MatMulOp):
            ...
            rendered = templates["matmul"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                input1=params["input1"],
        ...
        if isinstance(op, MatMulIntegerOp):
            ...
            rendered = templates["matmul_integer"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                input1=params["input1"],
        ...
        if isinstance(op, EinsumOp):
            ...
            if op.kind == EinsumKind.REDUCE_ALL:
                ...
                input_loop_bounds = tuple(
                    CEmitter._shape_dim_exprs(input_shapes[0], input_dim_names[0])
                )
                if input_loop_vars:
            ...
            rendered = templates["einsum"].render(
                model_name=model.name,
                op_name=op_name,
                params=param_decls,
                dim_args=dim_args,
        ...
        if isinstance(op, GemmOp):
            ...
            c_suffix = self._param_array_suffix(c_shape) if c_shape is not None else ""
            param_decls = self._build_param_decls(
                [
            ...
            alpha_literal = CEmitter._format_literal(dtype, self._derived(op, "alpha"))
            beta_literal = CEmitter._format_literal(dtype, self._derived(op, "beta"))
            ...
            if c_shape is None:
                ...
                c_dim0 = c_shape[0]
                c_dim1 = c_shape[1]
            rendered = templates["gemm"].render(
                model_name=model.name,
                op_name=op_name,
        ...
        if isinstance(op, AttentionOp):
            ...
            rendered = templates["attention"].render(
                model_name=model.name,
                op_name=op_name,
        ...
        if isinstance(op, RotaryEmbeddingOp):
            ...
            rendered = templates["rotary_embedding"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
        ...
        if isinstance(op, ConvOp):
            ...
            rendered = templates["conv"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
        ...
        if isinstance(op, ConvIntegerOp):
            ...
            if params["w_zero_point"]:
                ...
                w_zero_expr = "0"
            rendered = templates["conv_integer"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                weights=params["weights"],
        ...
        if isinstance(op, QLinearConvOp):
            ...
            rendered = templates["qlinear_conv"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
        ...
        if isinstance(op, Col2ImOp):
            ...
            rendered = templates["col2im"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                output=params["output"],
        ...
        if isinstance(op, ConvTransposeOp):
            ...
            rendered = templates["conv_transpose"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                weights=params["weights"],
        ...
        if isinstance(op, DeformConvOp):
            ...
            rendered = templates["deform_conv"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                weights=params["weights"],
        ...
        if isinstance(op, AveragePoolOp):
            ...
            rendered = templates["avg_pool"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                output=params["output"],
        ...
        if isinstance(op, LpPoolOp):
            ...
            rendered = templates["lp_pool"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                output=params["output"],
        ...
        if isinstance(op, BatchNormOp):
            ...
            rendered = templates["batch_norm"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                scale=params["scale"],
                ...
                one_minus_momentum_literal=CEmitter._format_floating(
        ...
        if isinstance(op, LpNormalizationOp):
            ...
            rendered = templates["lp_norm"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                output=params["output"],
        ...
        if isinstance(op, InstanceNormalizationOp):
            ...
            rendered = templates["instance_norm"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                scale=params["scale"],
        ...
        if isinstance(op, GroupNormalizationOp):
            ...
            rendered = templates["group_norm"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                scale=params["scale"],
        ...
        if isinstance(op, LayerNormalizationOp):
            ...
            mean_suffix = (
                self._param_array_suffix(
                    op.shape[: op.axis] + (1,) * len(op.normalized_shape)
                )
            ...
            rendered = templates["layer_norm"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                scale=params["scale"],
        ...
        if isinstance(op, MeanVarianceNormalizationOp):
            ...
            rendered = templates["mean_variance_norm"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                output=params["output"],
        ...
        if isinstance(op, RMSNormalizationOp):
            ...
            rendered = templates["rms_norm"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                scale=params["scale"],
        ...
        if isinstance(op, LstmOp):
            ...
            rendered = templates["lstm"].render(
                model_name=model.name,
                op_name=op_name,
                input_x=params["input_x"],
                input_w=params["input_w"],
        ...
        if isinstance(op, AdagradOp):
            params = self._shared_param_map(
                [
                    ...
                    *((f"acc{idx}", name) for idx, name in enumerate(op.accumulators)),
            ...
            rendered = templates["adagrad"].render(
                model_name=model.name,
                op_name=op_name,
                rate=params["rate"],
        ...
        if isinstance(op, MomentumOp):
            ...
            param_decls = self._build_param_decls(param_specs)
            rendered = templates["momentum"].render(
                model_name=model.name,
                op_name=op_name,
                rate=params["rate"],
                timestep=params["timestep"],
        ...
        if isinstance(op, SoftmaxOp):
            ...
            rendered = templates["softmax"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                output=params["output"],
        ...
        if isinstance(op, LogSoftmaxOp):
            ...
            rendered = templates["logsoftmax"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                output=params["output"],
        ...
        if isinstance(op, HardmaxOp):
            ...
            rendered = templates["hardmax"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                output=params["output"],
        ...
        if isinstance(op, NegativeLogLikelihoodLossOp):
            ...
            rendered = templates["nllloss"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
        ...
        if isinstance(op, SoftmaxCrossEntropyLossOp):
            ...
            rendered = templates["softmax_cross_entropy_loss"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                target=params["target"],
                ...
                output_suffix=output_suffix,
                log_prob_suffix=(
                    log_prob_suffix if op.log_prob_shape is not None else None
                ),
                n=op.n,
                c=op.c,
                d=op.d,
        ...
        if isinstance(op, MaxPoolOp):
            ...
            rendered = templates["maxpool"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                output=params["output"],
        ...
        if isinstance(op, ConcatOp):
            ...
            params = self._shared_param_map([*input_params, ("output", op.output)])
            input_names = tuple(
            ...
            rendered = templates["concat"].render(
                model_name=model.name,
                op_name=op_name,
                inputs=input_names,
                output=params["output"],
        ...
        if isinstance(op, ConcatFromSequenceOp):
            ...
            rendered = templates["concat_from_sequence"].render(
                op_name=op_name,
                dim_args=dim_args,
                params=param_decls,
        ...
        if isinstance(op, CompressOp):
            ...
            rendered = templates["compress"].render(
                model_name=model.name,
                op_name=op_name,
                data=params["data"],
                condition=params["condition"],
        ...
        if isinstance(op, GatherElementsOp):
            ...
            rendered = templates["gather_elements"].render(
                model_name=model.name,
                op_name=op_name,
                data=params["data"],
                indices=params["indices"],
        ...
        if isinstance(op, GatherOp):
            ...
            rendered = templates["gather"].render(
                model_name=model.name,
                op_name=op_name,
                data=params["data"],
                indices=params["indices"],
        ...
        if isinstance(op, GatherNDOp):
            ...
            rendered = templates["gather_nd"].render(
                model_name=model.name,
                op_name=op_name,
                data=params["data"],
                indices=params["indices"],
        ...
        if isinstance(op, ScatterElementsOp):
            ...
            rendered = templates["scatter_elements"].render(
                model_name=model.name,
                op_name=op_name,
                data=params["data"],
                indices=params["indices"],
        ...
        if isinstance(op, ScatterOp):
            ...
            rendered = templates["scatter"].render(
                model_name=model.name,
                op_name=op_name,
                data=params["data"],
                indices=params["indices"],
        ...
        if isinstance(op, ScatterNDOp):
            ...
            rendered = templates["scatter_nd"].render(
                model_name=model.name,
                op_name=op_name,
                data=params["data"],
                indices=params["indices"],
        ...
        if isinstance(op, TensorScatterOp):
            ...
            rendered = templates["tensor_scatter"].render(
                model_name=model.name,
                op_name=op_name,
        ...
        if isinstance(op, IdentityOp):
            ...
            if c_type == "char":
                ...
                rendered = templates["identity"].render(
                    model_name=model.name,
                    op_name=op_name,
                    input0=params["input0"],
                    output=params["output"],
        ...
        if isinstance(op, BernoulliOp):
            ...
            rendered = templates["bernoulli"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                output=params["output"],
        ...
        if isinstance(op, DropoutOp):
            ...
            rendered = templates["dropout"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
        ...
        if isinstance(op, EyeLikeOp):
            ...
            rendered = templates["eye_like"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                output=params["output"],
        ...
        if isinstance(op, TriluOp):
            ...
            rendered = templates["trilu"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                output=params["output"],
        ...
        if isinstance(op, TileOp):
            ...
            rendered = templates["tile"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                output=params["output"],
        ...
        if isinstance(op, CenterCropPadOp):
            ...
            for dim in reversed(input_shape_raw):
                ...
                stride *= dim
            input_strides = list(reversed(input_strides))
            rendered = templates["center_crop_pad"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
        ...
        if isinstance(op, DepthToSpaceOp):
            ...
            rendered = templates["depth_to_space"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                output=params["output"],
        ...
        if isinstance(op, SpaceToDepthOp):
            ...
            rendered = templates["space_to_depth"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                output=params["output"],
        ...
        if isinstance(op, SliceOp):
            ...
            if op.starts is not None and op.steps is not None:
                ...
                rendered = templates["slice"].render(
                    model_name=model.name,
                    op_name=op_name,
                    input0=name_params["input0"],
                    output=name_params["output"],
            ...
            rendered = templates["slice_dynamic"].render(
                model_name=model.name,
                op_name=op_name,
                params=params,
                input0=name_params["input0"],
        ...
        if isinstance(op, ResizeOp):
            ...
            rendered = templates["resize"].render(
                model_name=model.name,
                op_name=op_name,
                params=params,
                input0=name_params["input0"],
                ...
                extrapolation_value=CEmitter._format_double(op.extrapolation_value),
        ...
        if isinstance(op, GridSampleOp):
            ...
            output_loop_vars = CEmitter._loop_vars(output_shape)
            rendered = templates["grid_sample"].render(
                model_name=model.name,
                op_name=op_name,
                params=params,
                input0=op.input0,
        ...
        if isinstance(op, AffineGridOp):
            ...
            rendered = templates["affine_grid"].render(
                model_name=model.name,
                op_name=op_name,
                params=params,
                theta=op.theta,
        ...
        if isinstance(op, ConstantOfShapeOp):
            ...
            rendered = templates["constant_of_shape"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                output=params["output"],
        ...
        if isinstance(op, ShapeOp):
            ...
            rendered = templates["shape"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                output=params["output"],
        ...
        if isinstance(op, SizeOp):
            ...
            rendered = templates["size"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                output=params["output"],
        ...
        if isinstance(op, OptionalHasElementOp):
            ...
            rendered = templates["optional_has_element"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
        ...
        if isinstance(op, OptionalGetElementOp):
            ...
            rendered = templates["optional_get_element"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                output=params["output"],
        ...
        if isinstance(op, NonZeroOp):
            ...
            rendered = templates["nonzero"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                output=params["output"],
        ...
        if isinstance(op, UniqueOp):
            ...
            rendered = templates["unique"].render(
                model_name=model.name,
                op_name=op_name,
                params=param_decls,
                dim_args=dim_args,
        ...
        if isinstance(op, NonMaxSuppressionOp):
            ...
            rendered = templates["nonmax_suppression"].render(
                model_name=model.name,
                op_name=op_name,
                boxes=params["boxes"],
        ...
        if isinstance(op, ExpandOp):
            ...
            rendered = templates["expand"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                output=params["output"],
        ...
        if isinstance(op, CumSumOp):
            ...
            rendered = templates["cumsum"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
        ...
        if isinstance(op, STFTOp):
            ...
            rendered = templates["stft"].render(
                model_name=model.name,
                op_name=op_name,
                params=param_decls,
        ...
        if isinstance(op, RangeOp):
            ...
            rendered = templates["range"].render(
                model_name=model.name,
                op_name=op_name,
                start=params["start"],
                limit=params["limit"],
        ...
        if isinstance(op, LoopRangeOp):
            ...
            rendered = templates["loop_range"].render(
                model_name=model.name,
                op_name=op_name,
        ...
        if isinstance(op, LoopSequenceInsertOp):
            ...
            rendered = templates["loop_sequence_insert"].render(
                op_name=op_name,
                dim_args=dim_args,
                params=param_decls,
                trip_count=params["trip_count"],
        ...
        if isinstance(op, BlackmanWindowOp):
            ...
            rendered = templates["blackman_window"].render(
                model_name=model.name,
                op_name=op_name,
                size=params["size"],
                output=params["output"],
        ...
        if isinstance(op, HammingWindowOp):
            ...
            rendered = templates["hamming_window"].render(
                model_name=model.name,
                op_name=op_name,
                size=params["size"],
                output=params["output"],
        ...
        if isinstance(op, HannWindowOp):
            ...
            rendered = templates["hann_window"].render(
                model_name=model.name,
                op_name=op_name,
                size=params["size"],
                output=params["output"],
        ...
        if isinstance(op, MelWeightMatrixOp):
            ...
            compute_type = "double" if compute_dtype == ScalarType.F64 else "float"
            rendered = templates["mel_weight_matrix"].render(
                model_name=model.name,
                op_name=op_name,
        ...
        if isinstance(op, OneHotOp):
            ...
            rendered = templates["one_hot"].render(
                model_name=model.name,
                op_name=op_name,
                indices=params["indices"],
                depth=params["depth"],
        ...
        if isinstance(op, TfIdfVectorizerOp):
            ...
            rendered = templates["tfidf_vectorizer"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                output=params["output"],
        ...
        if isinstance(op, StringConcatOp):
            ...
            rendered = templates["string_concat"].render(
                op_name=op_name,
                dim_args=dim_args,
                params=param_decls,
                shape=shape,
        ...
        if isinstance(op, StringNormalizerOp):
            ...
            rendered = templates["string_normalizer"].render(
                op_name=op_name,
                dim_args=dim_args,
                params=param_decls,
                input0=params["input0"],
        ...
        if isinstance(op, LabelEncoderOp):
            ...
            rendered = templates["label_encoder"].render(
                op_name=op_name,
                dim_args=dim_args,
                params=param_decls,
                input0=params["input0"],
        ...
        if isinstance(op, StringSplitOp):
            ...
            rendered = templates["string_split"].render(
                op_name=op_name,
                dim_args=dim_args,
                params=param_decls,
                input0=params["input0"],
        ...
        if isinstance(op, TreeEnsembleOp):
            ...
            rendered = templates["tree_ensemble"].render(
                op_name=op_name,
                dim_args=dim_args,
                params=param_decls,
                input0=params["input0"],
        ...
        if isinstance(op, TreeEnsembleClassifierOp):
            ...
            rendered = templates["tree_ensemble_classifier"].render(
                op_name=op_name,
                dim_args=dim_args,
                params=param_decls,
                input0=params["input0"],
        ...
        if isinstance(op, ReverseSequenceOp):
            ...
            rendered = templates["reverse_sequence"].render(
                op_name=op_name,
                dim_args=dim_args,
                params=param_decls,
                input0=params["input0"],
        ...
        if isinstance(op, SequenceAtOp):
            ...
            position_dtype = self._ctx_dtype(op.position)
            tensor_suffix = self._param_array_suffix(
                output_shape, _dim_names_for(op.output)
            )
            input_suffix = self._param_array_suffix(
                self._sequence_storage_shape(op.input_sequence),
                _dim_names_for(op.input_sequence),
            )
            param_decls = self._build_param_decls(
                [
                    (
                    ...
                    (
                        ...
                        self._param_array_suffix((1,)),
                        True,
                    ),
                    (params["output"], c_type, tensor_suffix, False),
                ]
            )
            rendered = templates["sequence_at"].render(
                op_name=op_name,
                dim_args=dim_args,
                params=param_decls,
                input_sequence=params["input_sequence"],
                ...
                output=params["output"],
                element_count=CEmitter._element_count_expr(output_shape),
                c_type=c_type,
            ).rstrip()
            return with_node_comment(rendered)
        if isinstance(op, SequenceIdentityOp):
            ...
            rendered = templates["sequence_identity"].render(
                op_name=op_name,
                dim_args=dim_args,
        ...
        if isinstance(op, SequenceLengthOp):
            ...
            rendered = templates["sequence_length"].render(
                op_name=op_name,
                dim_args=dim_args,
                params=param_decls,
                input_sequence=params["input_sequence"],
        ...
        if isinstance(op, SequenceConstructOp):
            ...
            param_decls = self._build_param_decls(param_specs)
            rendered = templates["sequence_construct"].render(
                op_name=op_name,
                dim_args=dim_args,
                params=param_decls,
                inputs=tuple(
        ...
        if isinstance(op, SequenceEmptyOp):
            ...
            tensor_suffix = self._param_array_suffix(
                ...
                _dim_names_for(op.output_sequence),
            )
            params = self._shared_param_map([("output_sequence", op.output_sequence)])
            param_decls = self._build_param_decls(
                [
                    (
                        params["output_sequence"],
            ...
            rendered = templates["sequence_empty"].render(
                op_name=op_name,
                dim_args=dim_args,
                params=param_decls,
                output_sequence=params["output_sequence"],
        ...
        if isinstance(op, SequenceEraseOp):
            ...
            param_decls = self._build_param_decls(param_specs)
            rendered = templates["sequence_erase"].render(
                op_name=op_name,
                dim_args=dim_args,
                params=param_decls,
                input_sequence=params["input_sequence"],
        ...
        if isinstance(op, SequenceInsertOp):
            ...
            sequence_suffix = self._param_array_suffix(
                self._sequence_storage_shape(op.input_sequence),
                _dim_names_for(op.input_sequence),
            )
            position_dtype = (
            ...
            param_decls = self._build_param_decls(param_specs)
            rendered = templates["sequence_insert"].render(
                op_name=op_name,
                dim_args=dim_args,
                params=param_decls,
                input_sequence=params["input_sequence"],
                ...
                position=params["position"],
                output_sequence=params["output_sequence"],
                element_count=CEmitter._element_count_expr(tensor_shape),
                c_type=c_type,
            ).rstrip()
            return with_node_comment(rendered)
        if isinstance(op, IfOptionalSequenceConstOp):
            elem_type = self._ctx_sequence_elem_type(op.output_sequence)
            elem_shape = self._codegen_shape(elem_type.shape)
            true_has_values = len(op.true_values) > 0
            false_has_values = len(op.false_values) > 0
            sequence_suffix = self._param_array_suffix(
                self._sequence_storage_shape(op.output_sequence),
                _dim_names_for(op.output_sequence),
            )
            cond_shape = self._codegen_shape(self._ctx_shape(op.cond))
            params = self._shared_param_map(
                [
                    ("cond", op.cond),
                    ("output_sequence", op.output_sequence),
            ...
            param_specs = [
                (
                    params["cond"],
                    ScalarType.BOOL.c_type,
                    self._param_array_suffix(cond_shape),
                    True,
                ),
                (
                    params["output_sequence"],
            ...
            elem_count = CEmitter._element_count(elem_shape)
            lines = [f"void {op_name}({dim_args}{', '.join(param_decls)}) {{"]
            if true_has_values:
                true_literals = [
                    CEmitter._format_literal(elem_type.dtype, value)
                    for value in op.true_values
                ]
                lines.append(
            ...
            if op.true_present:
                ...
                if true_has_values:
                    loop_vars = self._loop_vars(elem_shape)
                    for depth, var in enumerate(loop_vars):
                        lines.append(
                            f"    for (idx_t {var} = 0; {var} < {elem_shape[depth]}; ++{var}) {{"
                        )
                    index_expr = self._index_expr(elem_shape, loop_vars)
            ...
            if params["output_present"] is not None:
                lines.append(
                    f"    *{params['output_present']} = {'1' if op.true_present else '0'};"
                )
            lines.append("} else {")
            if op.false_present:
                lines.append(
            ...
            if params["output_present"] is not None:
                lines.append(
                    f"    *{params['output_present']} = {'1' if op.false_present else '0'};"
                )
            lines.append("}")
            lines.append("}")
            return with_node_comment(_format_c_indentation("\n".join(lines)))
        if isinstance(op, SplitToSequenceOp):
            input_shape = self._ctx_shape(op.input0)
            if op.keepdims:
            ...
            for dim in input_shape[op.axis + 1 :]:
                inner *= dim
            split_len = (
                self._ctx_shape(op.split)[0]
                if op.split is not None and len(self._ctx_shape(op.split)) == 1
                else 0
            )
            rendered = templates["split_to_sequence"].render(
                op_name=op_name,
                dim_args=dim_args,
                params=param_decls,
                input0=params["input0"],
                split=params["split"],
                split_dtype=(
                    self._ctx_dtype(op.split).c_type
        ...
        if isinstance(op, SplitOp):
            ...
            params = self._shared_param_map([("input0", op.input0), *output_params])
            output_names = tuple(
                params[f"output_{index}"] for index in range(len(op.outputs))
            )
            ...
            rendered = templates["split"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
        ...
        if isinstance(op, CastOp):
            ...
            shape = CEmitter._shape_dim_exprs(output_shape_raw, output_dim_names)
            loop_vars = CEmitter._loop_vars(output_shape_raw)
            array_suffix = self._param_array_suffix(output_shape_raw, output_dim_names)
            param_decls = self._build_param_decls(
                [
            ...
            rendered = templates["cast"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                output=params["output"],
        ...
        if isinstance(op, QuantizeLinearOp):
            ...
            if params["zero_point"]:
                ...
                zero_expr = "0"
            rendered = templates["quantize_linear"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                scale=params["scale"],
        ...
        if isinstance(op, DequantizeLinearOp):
            ...
            if op.axis is None:
                ...
                scale_indices[op.axis] = f"({loop_vars[op.axis]}) / {op.block_size}"
            ...
            if params["zero_point"]:
                ...
                zero_expr = "0"
            rendered = templates["dequantize_linear"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                scale=params["scale"],
        ...
        if isinstance(op, DynamicQuantizeLinearOp):
            ...
            rendered = templates["dynamic_quantize_linear"].render(
                model_name=model.name,
                op_name=op_name,
                params=param_decls,
                compute_type=compute_type,
        ...
        if isinstance(op, QLinearAddOp):
            ...
            input0_index_expr = CEmitter._broadcast_index_expr(
                params["input0"], op.input0_shape, op.output_shape, output_loop_vars
            )
            input1_index_expr = CEmitter._broadcast_index_expr(
                params["input1"], op.input1_shape, op.output_shape, output_loop_vars
            )
            input0_suffix = self._param_array_suffix(op.input0_shape)
            input1_suffix = self._param_array_suffix(op.input1_shape)
            input0_scale_suffix = self._param_array_suffix(op.input0_scale_shape)
            input1_scale_suffix = self._param_array_suffix(op.input1_scale_shape)
            output_scale_suffix = self._param_array_suffix(op.output_scale_shape)
            ...
            scale_index = "0"
            rendered = templates["qlinear_add"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                input1=params["input1"],
        ...
        if isinstance(op, QLinearMulOp):
            ...
            scale_index = "0"
            rendered = templates["qlinear_mul"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                input1=params["input1"],
        ...
        if isinstance(op, QLinearAveragePoolOp):
            ...
            rendered = templates["qlinear_avg_pool"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                input_scale=params["input_scale"],
        ...
        if isinstance(op, QLinearMatMulOp):
            ...
            if op.dtype.is_signed:
                ...
                min_literal = "0.0"
                max_literal = "255.0"
            rendered = templates["qlinear_matmul"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
                input1=params["input1"],
        ...
        if isinstance(op, QLinearSoftmaxOp):
            ...
            rendered = templates["qlinear_softmax"].render(
                model_name=model.name,
                op_name=op_name,
                params=param_decls,
                compute_type=compute_type,
        ...
        if isinstance(op, ClipOp):
            ...
            output_dtype = self._ctx_dtype(op.output)
            min_shape = (
                self._ctx_shape(op.input_min) if op.input_min is not None else None
            )
            max_shape = (
            ...
            min_suffix = (
                self._param_array_suffix(min_shape, _dim_names_for(op.input_min))
                if min_shape is not None
                else ""
            )
            max_suffix = (
                self._param_array_suffix(max_shape, _dim_names_for(op.input_max))
            ...
            rendered = templates["clip"].render(
                model_name=model.name,
                op_name=op_name,
                input0=params["input0"],
        ...
        if isinstance(op, UnaryOp):
            ...
            rendered = templates["unary"].render(
                **common,
                input0=params["input0"],
                output=params["output"],
    ...
    def _op_inputs(
        ...
        if isinstance(op, EinsumOp):
            return tuple(
                (name, shape) for name, shape in zip(op.inputs, op.input_shapes)
            )
        if isinstance(op, UnaryOp):
        ...
        if isinstance(op, QuantizeLinearOp):
            scale_shape = (
                () if op.axis is None else (self._ctx_shape(op.input0)[op.axis],)
            )
