...
def lower_tree_ensemble_classifier(
    ...
    if len(label_shape) != 1 or label_shape[0] != input_shape[0]:
        raise UnsupportedOpError(
            f"{node.op_type} label output shape must be ({input_shape[0]},)"
        )
    ...
    class_labels = _as_int_tuple(
        attrs["classlabels_int64s"], name="classlabels_int64s", node=node
    )
    ...
    return TreeEnsembleClassifierOp(
        ...
        node_tree_ids=_as_int_tuple(
            attrs["nodes_treeids"], name="nodes_treeids", node=node
        ),
        node_node_ids=_as_int_tuple(
            attrs["nodes_nodeids"], name="nodes_nodeids", node=node
        ),
        ...
        node_values=_as_float_tuple(
            attrs["nodes_values"], name="nodes_values", node=node
        ),
        ...
        class_tree_ids=_as_int_tuple(
            attrs["class_treeids"], name="class_treeids", node=node
        ),
        class_node_ids=_as_int_tuple(
            attrs["class_nodeids"], name="class_nodeids", node=node
        ),
        class_ids=_as_int_tuple(attrs["class_ids"], name="class_ids", node=node),
        class_weights=_as_float_tuple(
            attrs["class_weights"], name="class_weights", node=node
        ),
