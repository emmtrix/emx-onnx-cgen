...
from ..ir.ops import MatMulIntegerOp
...
def resolve_matmul_integer_spec(graph: Graph, node: Node) -> MatMulIntegerSpec:
    if len(node.inputs) not in {2, 3, 4} or len(node.outputs) != 1:
        raise UnsupportedOpError("MatMulInteger must have 2 to 4 inputs and 1 output")
    ...
    if len(input0_shape) < 1 or len(input1_shape) < 1:
        raise UnsupportedOpError(
            "MatMulInteger inputs must be at least 1D, "
    ...
    if expected_output_shape != output_shape:
        raise ShapeInferenceError(
            "MatMulInteger output shape must be "
            f"{output_shape}, got {expected_output_shape}"
        )
    return MatMulIntegerSpec(
...
def _broadcast_batch_shapes(
    ...
    for left_dim, right_dim in zip(left_padded, right_padded):
        if not (left_dim == right_dim or left_dim == 1 or right_dim == 1):
            raise ShapeInferenceError(
                "MatMulInteger batch dimensions must be broadcastable, "
                f"got {left} x {right}"
            )
...
def _ensure_scalar_input(
    ...
    if shape not in {(), (1,)}:
        raise UnsupportedOpError(
            f"MatMulInteger {label} must be scalar, got shape {shape}"
        )
    return shape


@register_lowering("MatMulInteger")
def lower_matmul_integer(graph: Graph, node: Node) -> MatMulIntegerOp:
    ...
    if input0_dtype not in {ScalarType.U8, ScalarType.I8}:
        raise UnsupportedOpError("MatMulInteger supports uint8/int8 A input only")
    if input1_dtype not in {ScalarType.U8, ScalarType.I8}:
        raise UnsupportedOpError("MatMulInteger supports uint8/int8 B input only")
    if output_dtype != ScalarType.I32:
        raise UnsupportedOpError("MatMulInteger expects int32 outputs only")
    input0_zero_point: str | None = None
    input1_zero_point: str | None = None
    input0_zero_shape: tuple[int, ...] | None = None
    input1_zero_shape: tuple[int, ...] | None = None
    if len(node.inputs) >= 3 and node.inputs[2]:
        ...
        input0_zero_dtype = _value_dtype(graph, input0_zero_point, node)
        if input0_zero_dtype != input0_dtype:
            raise UnsupportedOpError(
                "MatMulInteger a_zero_point dtype must match A input dtype"
            )
    if len(node.inputs) >= 4 and node.inputs[3]:
        input1_zero_point = node.inputs[3]
        ...
        input1_zero_dtype = _value_dtype(graph, input1_zero_point, node)
        if input1_zero_dtype != input1_dtype:
            raise UnsupportedOpError(
                "MatMulInteger b_zero_point dtype must match B input dtype"
            )
