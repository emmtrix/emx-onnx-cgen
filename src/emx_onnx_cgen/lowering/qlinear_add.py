...
def _ensure_scalar_input(
    ...
    if shape not in {(), (1,)}:
        raise UnsupportedOpError(
            f"QLinearAdd {label} must be scalar, got shape {shape}"
        )
    return shape
...
def _ensure_scale_dtype(dtype: ScalarType, label: str) -> None:
    if not dtype.is_float:
        raise UnsupportedOpError(f"QLinearAdd {label} must be float16/float/double")


@register_lowering("QLinearAdd")
def lower_qlinear_add(graph: Graph, node: Node) -> QLinearAddOp:
    if len(node.inputs) != 8 or len(node.outputs) != 1:
        raise UnsupportedOpError("QLinearAdd must have 8 inputs and 1 output")
    ...
    if output_dtype not in {ScalarType.U8, ScalarType.I8}:
        raise UnsupportedOpError("QLinearAdd supports uint8/int8 outputs only")
    ...
    if input0_zero_dtype != input0_dtype:
        raise UnsupportedOpError("QLinearAdd a_zero_point dtype must match a")
    if input1_zero_dtype != input1_dtype:
        raise UnsupportedOpError("QLinearAdd b_zero_point dtype must match b")
    if output_zero_dtype != output_dtype:
        raise UnsupportedOpError("QLinearAdd y_zero_point dtype must match y")
