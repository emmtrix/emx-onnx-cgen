...
from ..ir.ops import NhwcMaxPoolOp
...
def _resolve_nhwc_maxpool_spec(graph: Graph, node: Node) -> _NhwcMaxPoolSpec:
    if len(node.inputs) != 1 or len(node.outputs) != 1:
        raise UnsupportedOpError("NhwcMaxPool must have 1 input and 1 output")
    supported_attrs = {
        "auto_pad",
        "ceil_mode",
        "dilations",
    ...
    if set(node.attrs) - supported_attrs:
        raise UnsupportedOpError("NhwcMaxPool has unsupported attributes")
    storage_order = int(node.attrs.get("storage_order", 0))
    if storage_order != 0:
        raise UnsupportedOpError("NhwcMaxPool supports storage_order=0 only")
    kernel_shape = node.attrs.get("kernel_shape")
    if kernel_shape is None:
        raise UnsupportedOpError("NhwcMaxPool requires kernel_shape")
    kernel_shape = tuple(int(value) for value in kernel_shape)
    input_shape = _value_shape(graph, node.inputs[0], node)
    if len(input_shape) < 3:
        raise UnsupportedOpError("NhwcMaxPool expects NHWC inputs with spatial dims")
    spatial_rank = len(input_shape) - 2
    if spatial_rank not in {1, 2, 3}:
        raise UnsupportedOpError("NhwcMaxPool supports 1D/2D/3D inputs only")
    if len(kernel_shape) != spatial_rank:
        raise ShapeInferenceError(
            f"NhwcMaxPool kernel_shape must have {spatial_rank} dims, "
            f"got {kernel_shape}"
        )
    strides = tuple(
    ...
    if auto_pad in ("", "NOTSET"):
        ...
        raise UnsupportedOpError("NhwcMaxPool has unsupported auto_pad mode")
    ceil_mode = int(node.attrs.get("ceil_mode", 0))
    if ceil_mode not in (0, 1):
        raise UnsupportedOpError("NhwcMaxPool supports ceil_mode=0 or 1 only")
    out_spatial = []
    for dim, stride, dilation, kernel, pad_start, pad_finish in zip(
        ...
        if out_dim < 0:
            raise ShapeInferenceError("NhwcMaxPool output shape must be non-negative")
        out_spatial.append(out_dim)
    expected_output_shape = (batch, *out_spatial, channels)
    output_shape = _value_shape(graph, node.outputs[0], node)
    if output_shape != expected_output_shape:
        raise ShapeInferenceError(
            f"NhwcMaxPool output shape must be {expected_output_shape}, "
            f"got {output_shape}"
        )
    pads = (*pad_begin, *pad_end)
    return _NhwcMaxPoolSpec(
        batch=batch,
        channels=channels,
...
@register_lowering("NhwcMaxPool")
def lower_nhwc_maxpool(graph: Graph, node: Node) -> NhwcMaxPoolOp:
    if len(node.inputs) != 1 or len(node.outputs) != 1:
        raise UnsupportedOpError("NhwcMaxPool must have 1 input and 1 output")
    op_dtype = _node_dtype(graph, node, node.inputs[0], node.outputs[0])
    if op_dtype == ScalarType.BOOL:
        raise UnsupportedOpError("NhwcMaxPool supports numeric inputs only")
    spec = _resolve_nhwc_maxpool_spec(graph, node)
    return NhwcMaxPoolOp(
