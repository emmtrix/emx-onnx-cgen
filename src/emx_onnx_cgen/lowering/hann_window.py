...
from ..ir.ops import HannWindowOp
...
@register_lowering("HannWindow")
def lower_hann_window(graph: Graph, node: Node) -> HannWindowOp:
    if len(node.inputs) != 1 or len(node.outputs) != 1:
        raise UnsupportedOpError("HannWindow must have 1 input and 1 output")
    size_shape = value_shape(graph, node.inputs[0], node)
    if not _is_scalar_shape(size_shape):
        raise UnsupportedOpError("HannWindow size input must be a scalar")
    input_dtype = value_dtype(graph, node.inputs[0], node)
    if input_dtype not in _SUPPORTED_INPUT_DTYPES:
        raise UnsupportedOpError(
            f"HannWindow size input must be int32 or int64, got {input_dtype.onnx_name}"
        )
    output_shape = value_shape(graph, node.outputs[0], node)
    if len(output_shape) != 1:
        raise ShapeInferenceError("HannWindow output must be 1D")
    output_dtype = value_dtype(graph, node.outputs[0], node)
    if output_dtype not in _SUPPORTED_OUTPUT_DTYPES:
        raise UnsupportedOpError(
            f"HannWindow output dtype must be numeric, got {output_dtype.onnx_name}"
        )
    ...
    if output_datatype is not None:
        ...
        if attr_dtype != output_dtype:
            raise UnsupportedOpError(
                "HannWindow output_datatype does not match output dtype"
            )
    periodic = int(node.attrs.get("periodic", 1))
    if periodic not in {0, 1}:
        raise UnsupportedOpError("HannWindow periodic must be 0 or 1")
    size_value = _read_scalar_initializer(graph, node.inputs[0], node)
    if size_value is not None:
        if size_value < 0:
            raise ShapeInferenceError("HannWindow size must be non-negative")
        if output_shape[0] != size_value:
            raise ShapeInferenceError(
                "HannWindow output length does not match size input"
            )
    return HannWindowOp(
        size=node.inputs[0],
