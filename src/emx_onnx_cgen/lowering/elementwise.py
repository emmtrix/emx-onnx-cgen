...
def lower_selu(graph: Graph, node: Node) -> UnaryOp:
    if len(node.inputs) != 1 or len(node.outputs) != 1:
        raise UnsupportedOpError("Selu must have 1 input and 1 output")
    dtype = node_dtype(graph, node, *node.inputs, *node.outputs)
    if not dtype.is_float:
        raise UnsupportedOpError("Selu only supports floating-point inputs")
    for key in node.attrs:
        if key not in {"alpha", "gamma"}:
            raise UnsupportedOpError(f"Selu does not support attribute {key}")
    try:
        alpha = float(node.attrs.get("alpha", 1.6732632423543772))
    except (TypeError, ValueError) as exc:
        raise UnsupportedOpError("Selu alpha must be numeric") from exc
    try:
        gamma = float(node.attrs.get("gamma", 1.0507009873554805))
    except (TypeError, ValueError) as exc:
        raise UnsupportedOpError("Selu gamma must be numeric") from exc
    return UnaryOp(
        input0=node.inputs[0],
        output=node.outputs[0],
